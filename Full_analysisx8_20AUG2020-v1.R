...
ggplot(melt_deg, aes(y=Number, x=Dataset,fill=Direction))+geom_bar(stat='identity')+theme_light()+
   theme(axis.text.x = element_text(
      color="black",size=10, angle=90,vjust = .5, hjust=1))+ggtitle('DEG')
...
plot(DEGtable$SampleSize,DEGtable$Total)
plot(DEGtable$SampleSize,DEGtable$Up)
plot(DEGtable$SampleSize,DEGtable$Down)
...
EnhancedVolcano(mrsc,
                ...
                x = 'randomSummary',
                y = 'adjP',
                xlim = c(-4.2, 5),ylim=c(0,25), title = 'Metax8',subtitle = '',transcriptLabSize = 2,
                pCutoff = .05,FCcutoff = log2(1.5), drawConnectors = F)
...
EnhancedVolcano(mrsc,
                ...
                x = 'randomSummary',
                y = 'adjP',
                xlim = c(-4.2, 5), title = 'Metax8',subtitle = '',transcriptLabSize = 2,
                pCutoff = .05,FCcutoff = log2(1.5), drawConnectors = F)
...
getwd()
##up
source('../MetaProjects/MetaDEG_v01/Code/draw_forest_mod.R') # mod allows manual pick of axis length
for (i in 1:2){
...
p=ggplot(temp, aes(x=variable,y= -log10(value))) + geom_bar(position = 'dodge',stat='identity')+theme_light()+
   theme(axis.text.x = element_text(
      color='black',size=10, angle=90,vjust = .5, hjust=1))+ggtitle('Up DEG')+facet_wrap(~Description)+theme( strip.text.x = element_text(
         size = 12, color='black'       )     ) 
print(p)
}
rm(enr_ind_meta)
...
for (g in levels(mc$Desc_Group)){
   ...
   p=ggplot(temp, aes(x=variable,y= -log10(value))) + geom_bar(position = 'dodge',stat='identity')+theme_light()+
      theme(axis.text.x = element_text(
         color='black',size=10, angle=90,vjust = .5, hjust=1))+ggtitle('Down DEG')+facet_wrap(~Description)+theme( strip.text.x = element_text(
            size = 12, color='black'       )     ) 
   print(p)
}
